...
import {db, collection, addDoc, getDocs, storage} from '../Firebase/config';
...
//         }} onPress={() => {Linking.openURL("demo://app")}} title="Hello" />
//       </View>
//     );
//   };

export default function RedirectToPaymentApps() {
  ...
  const enterUPIManually = async() => {
    let response = await UPI.openLink("upi://pay?pa=9370472578@upi&pn=Miss%20ISHA%20PRASHANT%20SHIVALKAR&cu=INR&mode=02&purpose=00&orgid=189999&sign=MEUCIAGQ3NiFSpywYjDpOTDec5yZ5UDqvsXp+tlDFcZOFqydAiEAqgztdPGlqMbpnm11W3Cjg2d68HwPWqrB6ky1LM6Gves=");
    console.log("response : ", response)
    // Linking.openURL("upi://pay?pa=9370472578@upi&pn=Miss%20ISHA%20PRASHANT%20SHIVALKAR&cu=INR&mode=02&purpose=00&orgid=189999&sign=MEUCIAGQ3NiFSpywYjDpOTDec5yZ5UDqvsXp+tlDFcZOFqydAiEAqgztdPGlqMbpnm11W3Cjg2d68HwPWqrB6ky1LM6Gves=")
    // setDialogBoxVisibility(true);
  }
  const redirectToUPIAppManualIn = async (upi) => {
    console.log("UPI : ", upi);
    setDialogBoxVisibility(false);
    // const response = await Linking.openURL(`upi://pay?pa=${upi}&pn=VidyaBorkar&cu=INR`).then(response=>{console.log(response)});
    let UpiUrl = `upi://pay?pa=${upi}&pn=vidya borkar&tr=kdahskjahs27575fsdfasdas&am=1&mam=null&cu=INR`;
  ...
  useEffect(() => {
    ...
    if(barcodes[0])
    {
      ...
      // redirectToUPIAppUsingQR(barcodes[0].displayvalue+"");
      // setQRScannerVisibility(false);
    }
    // redirectToUPIAppUsingQR(barcodes.displayValue);
    // setQRScannerVisibility(false);
  ...
  const redirectToUPIAppUsingQR = async (upiLink) => {
    ...
    // let response = await UPI.openLink(upiLink);
    // console.log("response : ", response)
  }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.container2}>
            ...
            <DialogInput isDialogVisible={dialogBoxVisibility}
              title={"Payee's UPI ID"}
              message={"Enter correct UPI ID of Payee : "}
              hintInput ={"yourname@bankname"}
              submitInput={ (inputText) => {redirectToUPIAppManualIn(inputText)} }
              closeDialog={ () => {setDialogBoxVisibility(false)}}>
            </DialogInput>


            {/* <View style={styles.container}>
              <Button title="Show dialog" onPress={dialogBoxVisibility} />
              <Dialog.Container visible={visible}>
                <Dialog.Title>Payee's UPI ID</Dialog.Title>
                <Dialog.Description>
                  Enter correct UPI ID of Payee :
                </Dialog.Description>
                <Dialog.Button label="Cancel" onPress={ () => {setDialogBoxVisibility(false)}} />
                <Dialog.Button label="Submit" onPress={ (inputText) => {redirectToUPIAppManualIn(inputText)} } />
              </Dialog.Container>
            </View> */}
            {/* <Text style = { {color: '#9c27b0' }}>Hello</Text> */}
            <Button
              onPress={scanQR}
              title="Scan QR"
              color="blue"
            ...
            <Modal
              animationType="slide"
              transparent
              visible={device != null && QRScannerVisibility &&
                hasPermission}
              presentationStyle="overFullScreen"
              onDismiss={() => {
                setQRScannerVisibility(false);
            ...
            {/* {device != null && QRScannerVisibility &&
              hasPermission && (
            ...
            {/* <ScrollView horizontal={true} >
                <SquareView size={50} color="red" />
                <SquareView size={50} color="blue" />
                <SquareView size={50} color="green" />
                <SquareView size={50} color="yellow" />
                <SquareView size={50} color="gray" />
                <SquareView size={50} color="cyan" />
                <SquareView size={50} color="black" />
            </ScrollView> */ }
        </View>
        <View style={styles.container2}>
          ...
          >
            <View style={styles.viewWrapper}>
              <View style={styles.modalView}>
                ...
                >
                  <Text style={{color: darkGreen, fontSize: 15, marginTop: 30}}>
                    {' '}
                    Close{' '}
                  </Text>
                </TouchableOpacity>
